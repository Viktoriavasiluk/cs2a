...
public:
   ...
   static int arraySearch(Student array[], string keyFirst,
...
int main() {
   Student myClass[] = { Student("smith","fred", 95),
   ...
   StudentArrayUtilities::printArray("Array to be searched: ", myClass, arraySize);
   string first, last;
   int found;

   first = "pamela"; last = "stollings";
   found = StudentArrayUtilities::arraySearch(myClass, first, last, arraySize);
   if(found >= 0)
      cout << first + " " + last + " IS in list at position " << found;
   else
      cout << first + " " + last + " is NOT in list.";
   cout << endl;
...
int StudentArrayUtilities::arraySearch(Student array[], string keyFirst,
                                       string keyLast, int arraySize) {
   for(int k = 0; k < arraySize; k++)
      if(array[k].getLastName() == keyLast && array[k].getFirstName() == keyFirst)
